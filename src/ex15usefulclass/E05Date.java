...
public class E05Date {
	...
	public static void main(String[] args) {
		...
		//현재 날짜를 기준으로 과거/미래를 구하기 위해 add메서드를 사용.
		//두번째 인수가 음수면 과거의 날짜를 구할 수 있다.
		...
		/*
		올해 크리스마스까지 남은 날짜를 계산, 여기서는 두가지 서식의 날짜를 파싱하여
		구한 날짜 인스턴스를 계산한다. */
		//오늘날짜 : 0000-00-00형식
...
//		System.out.println(today+" "+future);
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			/*
			매개변수를 통해 전달된 2개의 문자열은 날짜의서식이 서로 다르다.
...
//			System.out.println(formatToday+" "+formatFuture);
			...
			즉, 100으로 나눈 후 뒤의 3자리를 제거하면 순수한 초가
