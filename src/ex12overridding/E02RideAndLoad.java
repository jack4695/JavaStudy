...
오버로딩 : 메서드명은 같지만 매개변수의 타입, 갯수, 순서가 다른 경우를
	말한다. 즉, ★서로 다른 메서드★이므로 상속받은 하위클래스에는 오버로딩
	된 모든 메서드가 포함된다.
오버라이딩 : 메서드명, 매개변수, 반환형까지 ★모두 동일한 메서드★를 상속관계
	에 있는 하위클래스에서 재정의 한 것을 말한다.
	이 경우 하위클래서 정의한 메서드가 상위클래스의 메서드를 가릭
...
class C extends B {
	...
	/*
	load메서드는 오버로딩으로 정의했으므로 매서드명만 동일할 뿐 매개변수의
